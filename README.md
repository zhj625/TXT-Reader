...
当前按纯文本阅读，不保留封面、图片、字体、CSS 排版或交互内容；不支持 DRM 加密书籍。EPUB 2 / 3 优先读取书内导航目录、书名和作者，目录缺失时使用章节标题；TXT 会识别常见中文及英文章节标题。两种格式均可在阅读页打开目录并跳转。原始文件最大 50 MiB，解压总大小最大 100 MiB，单个文字文档最大 10 MiB，最多 10000 个 ZIP 条目。解析失败不会添加书籍。

从 v0.2.1 升级时，已有书籍和阅读进度会自动保留。旧记录尚未保存目录和作者时，可重新导入同一个原文件补全这些信息，原有阅读进度不会被重置；信息已经完整的书籍仍按重复导入处理。
